...
<div class="container" id="about">
    ...
    <c:if test="${empty highBlogs}">
        <p>暂无文章</p>
    </c:if>
    <c:if test="${not empty highBlogs}">
        <c:forEach items="${highBlogs}" var="highBlog" begin="0" end="0">
            <div class="jumbotron p-4 p-md-5 text-white rounded">
                <div class="col-md-6 px-0">
                    <h1 class="display-4 font-italic">${highBlog.title}</h1>
                    <p class="lead my-3">${highBlog.content}</p>
                    <p class="lead mb-0"><a href="view.jsp?bid=${highBlog.bid}"
        ...
        <div class="row mb-2">
            <c:if test="${length>1}">
                <c:forEach items="${highBlogs}" var="highBlog" begin="1" end="2">
                    <div class="col-md-6">
                        <div class="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                            <div class="col p-4 d-flex flex-column position-static">
                                <strong class="d-inline-block mb-2 inline-title">Unicom</strong>
                                <h3 class="mb-0">${highBlog.title}</h3>
                                <div class="mb-1 text-muted">${highBlog.createTime}</div>
                                <p class="card-text mb-auto">${highBlog.content}</p>
                            </div>
                            <!--read more btn-->


                                <button  class="btn btn-primary justify-content-center align-items-center read-more" onclick="View(${highBlog.bid})">
                                    Read More
                                </button>
