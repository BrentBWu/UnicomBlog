...
public class UserService {
    ...
    public static Result<List<User>> followUserList(int uid) {
        ...
        for(int i = 0; i < res.size(); i++) {
            User user = res.get(i);
            System.out.println("name=" + user.getUserName() + ",pass=" + user.getUserPassword() + ",nickname="
                    + user.getNickName() + ",sts=" + user.getSts() + ",uid=" + user.getUid() + ",head="
                    + user.getHead_icon() + ",time=" + user.getCreateTime());
        }
        return result;
    }
